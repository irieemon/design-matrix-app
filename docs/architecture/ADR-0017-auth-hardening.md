...
| T-0017-A22 | component [F] | AC-SESSION-04 -- boot-time wall-clock upper bound: if nothing resolves within `MAX_AUTH_INIT_TIME` (15000ms), `useAuth` surfaces `isLoading === false` with a safe-default `currentUser === null` (NOT stuck-loading). Distinct from B05 (B05 is login-submit timeout; A22 is boot-init timeout). File under test: `src/hooks/useAuth.ts:419` getSession + `:582` onAuthStateChange wrapped in wall-clock timer | Wave A | Mock both `getSession` and `onAuthStateChange` to never resolve; advance 15001ms; assert `isLoading === false`, no unhandled rejection |
