/**
 * UUID Utilities for API (No Frontend Dependencies)
 *
 * Simplified UUID validation and sanitization for serverless functions
 * without dependencies on frontend logging infrastructure
 */
...
export function sanitizeUserId(userId: string | undefined | null): string | null {
  ...
  if (!isValidUUID(trimmed)) {
    console.warn('Invalid UUID format', { uuid: trimmed })
    return null
  }
...
export function legacyIdToUUID(legacyId: string): string {
  ...
  const legacyMapping: Record<string, string> = {
    ...
    'test-project-1': '00000000-0000-0000-0000-000000000001',
    'test-project-123': '00000000-0000-0000-0000-000000000123',
...
export function sanitizeProjectId(projectId: string | undefined | null): string | null {
  ...
  // Handle legacy test project IDs
  ...
  // Validate UUID format
  if (!isValidUUID(trimmed)) {
    console.warn('Invalid UUID format for project ID', { projectId: trimmed })
    return legacyIdToUUID(trimmed)
  }
