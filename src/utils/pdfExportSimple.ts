...
export const exportInsightsToPDF = (insights: any, ideaCount: number, project: Project | null = null, filesWithContent: any[] = []) => {
  try {
    ...
    // Refined footer with subtle elegance
    const addFooter = (pageNum: number) => {
      // Delicate separator line
      doc.setDrawColor(ash[0], ash[1], ash[2])
      doc.setLineWidth(0.5)
      doc.line(marginL, pageH - 45, pageW - marginR, pageH - 45)
      
      // Small Prioritas logo in footer
      drawPrioritasLogo(marginL, pageH - 40, 12)
      
      doc.setFontSize(7)
      doc.setTextColor(steel[0], steel[1], steel[2])
      doc.text('Generated by Prioritas', marginL + 20, pageH - 32)
      doc.text(new Date().toLocaleDateString('en-US') + ' • ' + new Date().toLocaleTimeString('en-US', {hour: '2-digit', minute:'2-digit'}), marginL + 20, pageH - 26)
      
      // Page number with sophisticated styling
      const pageText = `${pageNum}`
      const pageTextWidth = doc.getTextWidth(pageText)
      doc.setFontSize(8)
      doc.setTextColor(slate[0], slate[1], slate[2])
      doc.text(pageText, pageW - marginR - pageTextWidth, pageH - 29)
      
      // Confidentiality notice
      doc.setFontSize(6)
      doc.setTextColor(ash[0], ash[1], ash[2])
      const confText = 'CONFIDENTIAL'
      const confWidth = doc.getTextWidth(confText)
      doc.text(confText, pageW - marginR - confWidth, pageH - 22)
    }
    ...
    // Elegant section headers with subtle design
    const addSectionHeader = (text: string, fontSize: number = 16, accentColor: number[] = prioritasBlue) => {
      ...
      doc.setFontSize(fontSize)
      doc.setFont('helvetica', 'bold')
      doc.text(text, marginL + 8, yPos + 4)
    ...
    // Elegant information boxes with subtle styling
    const addInfoBox = (title: string, content: string, bgColor: number[] = pearl, titleColor: number[] = charcoal) => {
      ...
      doc.text(title, marginL + 20, yPos + 18)
      
      // Content
      doc.setFontSize(10)
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(slate[0], slate[1], slate[2])
      doc.text(lines, marginL + 20, yPos + 32)
    ...
    // Elegant subtitle with brand accent
    doc.setFontSize(16)
    doc.setTextColor(prioritasBlue[0], prioritasBlue[1], prioritasBlue[2])
    ...
    doc.setDrawColor(ash[0], ash[1], ash[2])
    doc.setLineWidth(0.5)
    doc.line(marginL, yPos, marginL + 120, yPos)
    yPos += 20

    // Project description in elegant box
    if (project && project.description) {
      addInfoBox('Project Overview', project.description)
    }

    // Analysis summary with refined styling
    const analysisInfo = `Comprehensive analysis of ${ideaCount} strategic initiatives${filesWithContent.length > 0 ? ` informed by ${filesWithContent.length} supporting documents` : ''}.`
    addInfoBox('Analysis Scope', analysisInfo)
    
    // Document sources with sophisticated presentation
    if (filesWithContent && filesWithContent.length > 0) {
      addSectionHeader('Document Analysis Sources', 14, steel)
      
      addText('This analysis incorporates insights from the following uploaded documents:', 11, 'normal', slate)
      yPos += 8
      
      filesWithContent.forEach((file) => {
        const fileName = file.original_name || file.name
        addBulletPoint(`${fileName} (${file.file_type})`, 0, steel)
      })
      
      const totalChars = filesWithContent.reduce((sum, file) => sum + (file.content_preview?.length || 0), 0)
      addText(`Total content analyzed: ${totalChars.toLocaleString()} characters`, 10, 'italic', steel)
      yPos += 15
    }
