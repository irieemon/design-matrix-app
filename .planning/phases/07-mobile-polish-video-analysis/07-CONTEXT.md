...
- **D-15:** **Add a new handler** `analyzeVideo` alongside the existing `analyzeImage` path. Research (07-RESEARCH.md finding #1) confirmed the existing `api/_lib/ai/analyzeImage.ts` accepts exactly one `imageUrl` — no batching. The new handler maps 6 frames to multiple `{type:'image'}` content parts in one `generateObject` call with a Zod schema enforcing `{summary, suggestedIdeas: IdeaCard[]}`. Prompt explicitly asks for "holistic summary of the video content based on these sampled frames" + "extracted idea suggestions in structured format".
